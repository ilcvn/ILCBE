  ...
  async translateHTML(htmlContent, fromLang = 'vi', toLang = 'zh-CN') {
    ...
    for (let i = 0; i < elements.length; i++) {
        ...
        if ($(elem).children().length === 0) {
            ...
            if (originalText) {
                try {
                    const translated = await translate(originalText, { from: fromLang, to: toLang});
            ...
            $(elem).contents().each(async (index, child) => {
                if (child.type === 'text') {
    ...
    $('img').each(async (index, img) => {
        ...
        if (altText) {
            try {
                const translatedAlt = await translate(altText, { from: fromLang, to: toLang });
    ...
    translatedHtml = translatedHtml.replace(/<\/?html>/g, '').replace(/<\/?head>/g, '').replace(/<\/?body>/g, '');
